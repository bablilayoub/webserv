...
<body class="bg-gray-100 min-h-screen flex items-center justify-center">
    <div class="bg-white p-8 rounded-lg shadow-lg min-w-[300px] max-w-[500px] w-full">
        <h1 class="text-2xl font-bold text-gray-800 text-center mb-4">Python File Upload</h1>
...
if message:
    if message_type == "success":
        print(f"""
        <div class="mt-4 p-4 text-sm rounded-lg bg-green-100 text-green-800">
            {message}
        </div>
        """)
    elif message_type == "error":
        print(f"""
        <div class="mt-4 p-4 text-sm rounded-lg bg-red-100 text-red-800">
            {message}
        </div>
        """)
